...
function factory (type, config, load, typed) {
  ...
  function FastMatrix(data, datatype) {
    ...
    if (data && data.isMatrix === true) {
      ...
      // nothing provided
      self = Array.call(self, self[_data].length) || self;
      self[_data] = new Arr([]);
      self[_shape] = [0];
      //self[_dtype] = datatype;
    }

    Object.setPrototypeOf(self, FastMatrix.prototype);
    reIndex(self, 0, 0, self);

    return self;
  }

  Object.setPrototypeOf(FastMatrix, Array);
  Object.setPrototypeOf(FastMatrix.prototype, []);
   ...
   */
  FastMatrix.prototype.subset = function (index, replacement, defaultValue) {
   ...
   */
  FastMatrix.prototype.set = function (index, value, defaultValue) {
    if (!isArray(index))
      throw new TypeError('Array expected');
    if (index.length < this._size.length)
      throw new DimensionError(index.length, this._size.length, '<');
  ...
  function _get (matrix, index) {
    ...
    if (isScalar) {
      ...
      return new FastMatrix(_getSubmatrix(matrix._data, index, size.length, 0), matrix._datatype);
    }
  }
   ...
   */
  function reIndex(m, level, k, subr) {
    var i, subarr = subr || Array(m[_shape][level]);

    if (level === m[_shape].length - 1) {
      return m[_data].subarray(k, k + m[_shape][level]);
    }

    for (i = 0; i < m[_shape][level]; i += 1) {
      subarr[i] = reIndex(m, level + 1, k + i*m[_shape][level + 1]);
    }
    return subarr;
  }
